...
| `retirementAllocationId`    | ID?                      | What this is moved into from the solved retirement year, and null where it stays put. Offered by default only on the kinds §8.3's bridge can reach, since an account locked until 59½ has those years to recover and derisking it at 45 costs growth for no protection. A portfolio that carries a household to retirement is rarely the one they live off afterwards: selling shares into a bad year is what sends people back to work, and holding less of them is the usual answer. Without this the projection earns an accumulation return through a whole decumulation and reports a plan lasting longer than it will (§6.2). |
